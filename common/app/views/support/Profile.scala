...
class ShareImage(overlayUrlParam: String, shouldIncludeOverlay: Boolean) extends Profile(width = Some(1200)) {
  ...
  override def resizeString: String = {
    if(shouldIncludeOverlay) {
      val params = Seq(widthParam, heightParam, qualityparam, autoParam, sharpParam, fitParam, dprParam, overlayAlignParam, overlayWidthParam, overlayUrlParam).filter(_.nonEmpty).mkString("&")
      s"?$params"
    } else {
      super.resizeString
    }
...
object EmailVideoImage extends Profile(width = Some(580), autoFormat = false) with OverlayBase64 {
  ...
  override def resizeString: String = {
    val params = Seq(widthParam, heightParam, qualityparam, autoParam, sharpParam, dprParam, overlayAlignParam, overlayUrlParam).filter(_.nonEmpty).mkString("&")
    s"?$params"
  }
}
